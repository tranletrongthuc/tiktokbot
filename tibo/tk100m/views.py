...
from tk100m.apps import Tk100MConfig as conf
from tk100m import helper
from tk100m.TikTokApi.tiktok import TikTokApi
from tk100m.objs import video_obj, combination_obj

# api = TikTokApi.get_instance(use_test_endpoints=True)
# api = TikTokApi.get_instance(custom_verifyFp=verify_FP,generate_static_did=True)

# mongo_cli = MongoClient(f"mongodb://{conf.mongodb_info['username']}:{conf.mongodb_info['password']}@{conf.mongodb_info['addr']}")
mongo_cli = MongoClient(conf.mongodb_connection_string['local'])
mongo_db = mongo_cli["tk100m"]
...
def get_video_info(list_video_objects, tag):
    ...
    for item in tqdm(list_video_objects, desc=f"Getting info for #{tag}: ", position=0, leave=True):
        ...
        video_info.local_download_path = f"{conf.storage_dir}/downloads/{video_info.id}.mp4"

        try:
            mongo_collection.update_one({'id':video_info.id},{'$set':video_info.create_datapoint()})
            print(f"Update {video_info.tk_id}")
        except:
            mongo_collection.insert_one(video_info.create_datapoint())
            print(f"Insert {video_info.tk_id}")
    ...
    result_report = {
        'most_view_count':most_view_count,
        'new_row_count':len(new_rows),
        'updated_row_count':len(existed_rows)
    }

    result_data = pd.DataFrame(new_rows + existed_rows).head(20).to_dict('index')
    return result_report, result_data
...
def download_videos(list_video_objects, tag):
    ...
    for item in tqdm(list_video_objects, desc=f"Downloading videos for #{tag}: ", position=0, leave=True):
        download_sub_dir = f"{conf.storage_dir}/downloads"
...
def get_trending(request):
    ...
    result_report, result_data = get_video_info(trending, 'trending')
...
def restore_conbine_videos(comb_ids_for_restore):
    if len(comb_ids_for_restore) == 0:
        if os.path.exists(f"{conf.storage_dir}/comb_ids_for_restore.txt"):
            with open(f"{conf.storage_dir}/comb_ids_for_restore.txt", 'r', encoding='utf-8') as restore_f:
    ...
    output_dir = f"{conf.storage_dir}/combinations"
...
def update_info(report_only = True):
    report_dir = f'{conf.storage_dir}/reports'

    # backup old video_infos file
    if not os.path.exists(f"{conf.storage_dir}/history"):
        os.mkdir(f"{conf.storage_dir}/history")
    shutil.copyfile(conf.video_description_path,
                    f"{conf.storage_dir}/history/video_descriptions_{time.strftime('%d_%m_%Y_%H_%M_%S')}.csv")
